...
public class TranslateVisitor implements ast.Visitor {
    ...
    private int index;
    private Hashtable<String, Integer> indexTable;  // 每个方法一个 indexTable，用来标记局部变量的索引值。普通方法，索引从 1 开始。
    ...
    public void visit(ast.Ast.Exp.Id e) {
        if (!e.isField) {
            int indexIdLocal = this.indexTable.get(e.id);
            ast.Ast.Type.T type = e.type;
            if (type.getNum() > 0)// a reference
                emit(new Aload(indexIdLocal));
            else
                emit(new Iload(indexIdLocal));
        } else {
            emit(new Aload(0));
            e.type.accept(this);
            emit(new GetField(this.classId, e.id, this.type));
        }
        return;
    }
    ...
    public void visit(ast.Ast.Exp.NewIntArray e) {
        e.exp.accept(this);
        emit(new NewIntArray(this.type));
    }
    ...
    public void visit(ast.Ast.Exp.Not e) {
        ...
        emit(new Ldc(1));
        emit(new Goto(endl));
        emit(new LabelJ(truel));
        emit(new Ldc(0));
        emit(new LabelJ(endl));
        return;
    }
    ...
    public void visit(ast.Ast.Stm.Assign s) {
        if (!s.id.isField) {
            ...
            emit(new Aload(0)); // load the "objref" in opcode stack.
            s.exp.accept(this);
            s.type.accept(this);
            emit(new PutField(this.classId, s.id.id, this.type));
        }
        return;
    }
    ...
    public void visit(ast.Ast.Stm.AssignArray s) {
        s.id.accept(this);
        s.index.accept(this); // index
        s.exp.accept(this); // value
        emit(new Iastore());
        return;
    }
