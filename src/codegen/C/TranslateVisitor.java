...
public class TranslateVisitor implements ast.Visitor{
	...
	public void visit(ast.Ast.Stm.AssignArray s) {
		...
		Exp.T temp = this.exp;
		s.index.accept(this);
		this.stm = new Stm.AssignArray(s.id, this.exp, temp);
		return;
	}
	...
	public void visit(ast.Ast.Method.MethodSingle m) {
		...
		//add vars in class
//		for (ast.Ast.Dec.T decSingle : currentClass.decs) {
//			Iterator<Dec.T> it = locals.iterator();
//			boolean hasVar = false;
//			for (; it.hasNext();) {
//				if (((ast.Ast.Dec.DecSingle) decSingle).id
//						.equals(((Dec.DecSingle) it.next()))) {
//					hasVar = true;
//				}
//			}
//			if(!hasVar){
//				((ast.Ast.Dec.DecSingle)decSingle).type.accept(this);
//				Dec.DecSingle classDec = new Dec.DecSingle(this.type, ((ast.Ast.Dec.DecSingle)decSingle).id);
//				locals.add(classDec);
//			}
//		}
		LinkedList<Tuple> currentClassFields = this.table.get(currentClassName).fields;
		for (Tuple fieldSingle : currentClassFields) {
			Iterator<Dec.T> it = locals.iterator();
			boolean hasVar = false;
			for (; it.hasNext();) {
				if (fieldSingle.id
			...
			if(!hasVar){
				Dec.DecSingle classDec = new Dec.DecSingle(fieldSingle.type,fieldSingle.id);
				locals.add(classDec);
			}
		}
